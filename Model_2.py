##########################################################################################
### Model_2.py: A simple ANN model with 3 phases (train, validation and test) and early stopping codition (based on
# val_loss) ###
...
def get_data():
    ...
    train_val_X, test_X, train_val_y, test_y = train_test_split(data_X, data_y, test_size=0.2, random_state=42)
    ...
    print(f'> Real ratio of train - validation - test data : {train_X.shape[0]/len(data_files)*100:.1f}% - {val_X.shape[0]/len(data_files)*100:.1f}% '
...
def model_train(train_X, train_y, val_X, val_y, test_X, test_y, model_hyperparams, results, result_dir):
    ...
    model.add(keras.layers.Dense(output_shape, activation='softplus'))
    ...
    history = model.fit(train_X, train_y,
                        ...
                        verbose=1,
                        shuffle=True,
                        callbacks=[test_loss, early_stopping, TimeHistory(), csv_logger, check_point])
...
def main():
    ...
    print(f'> Validation: - Loss MSE: {val_scores[0]:.3e}')
    print(f'              - Metric RMSE: {val_scores[1]:.3e}')
    print(f'              - Metric MAE: {val_scores[2]:.3e}')
    print(f'> Test: - Loss MSE: {test_scores[0]:.3e}')
    print(f'        - Metric RMSE: {test_scores[1]:.3e}')
    print(f'        - Metric MAE: {test_scores[2]:.3e}')

    # Statistics scores
    r2_scores = np.array([r2_score(train_y, predict_train_y), r2_score(val_y, predict_val_y), r2_score(test_y, predict_test_y)])
    ...
    print(f'> Validation: - R2 score: {r2_scores[1]:.3e}')
    print(f'              - Adj R2 score: {adj_r2_scores[1]:.3e}')
    print(f'> Test: - R2 score: {r2_scores[2]:.3e}')
    print(f'        - Adj R2 score: {adj_r2_scores[2]:.3e}')
    ...
    # Save result
    Model_report.append(['Model_2', model_hyperparams['Layer_opt'],
